...
async def generate_task(
    ...
    if audio:
        ...
        message = await transcribe_audio_to_text(
            audio_bytes, str(audio.filename)
        )
...
async def get_task(
    task_list_id: str, current_user: str = Depends(get_current_user)
):
    if not current_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_pending_tasks(current_user: str = Depends(get_current_user)):
    if not current_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="User token required",
